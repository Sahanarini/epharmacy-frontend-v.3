...
const OrdersTable = () => {
  ...
  const [image, setImage] = useState(null);  // Assuming you want to store an image
  const [imageId, setImageId] = useState(null);  // To store an image ID
  const [open, setOpen] = useState(false);  // For controlling open/close state
  ...
  const handleViewDocument = async (id) => {
    try {
      const response = await axios.get(`http://localhost:9952/api/prescription/${id}`, {
        responseType: 'arraybuffer', // important for handling binary data
      });
      const base64 = Buffer.from(response.data, 'binary').toString('base64');
      setImage(`data:image/jpeg;base64,${base64}`); // Adjust the MIME type as needed
      setImageId(id);
      setOpen(true);
    } catch (error) {
          ...
          <Table sx={{ minWidth: 800 }} aria-label="table in dashboard">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ textAlign: "center" }}>Prescription</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
